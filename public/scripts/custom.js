...
$(document).ready(function(){      
	...
	function init_template(){	
        ...
        if(!$('.offline-message').length){
            $('#page').append('<p class="offline-message bg-red2-dark color-white center-text uppercase ultrabold">인터넷 연결이 끊어졌습니다.</p>');
            $('#page').append('<p class="online-message bg-green1-dark color-white center-text uppercase ultrabold">인터넷에 연결되었습니다.</p>');
        }     
...
function open_calendar() {    
    var calendar = document.getElementById('ctl_calendar');
    //console.log(calendar.className);
    var cal_btn = document.getElementById('ctl_calendar_btn');
    /*
    var inject_code = '<div id="ctl_tmp1">추가 예정</div>';
    */
    if (cal_btn.innerHTML.includes('plus')){
        calendar.className = calendar.className.replace(" ctl-cal-hidden", "");
        cal_btn.innerHTML = '<i class="fas fa-minus-square"></i>';
    }else{
        calendar.className += " ctl-cal-hidden";
        cal_btn.innerHTML = '<i class="fas fa-plus-square"></i>';
    }
}
